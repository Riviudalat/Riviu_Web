...
- Landing được ghép từ các section trong `apps/web/components/`: header, hero, marquee, stats, services, channel-network, pricing (tab Facebook / Bài đăng / TikTok + slide từng gói), process, impact, faq, contact (`POST /api/contact` rồi mới hiện cảm ơn; SĐT bắt buộc), footer, chat-widget. Testimonials mock và section ứng dụng (Features) đã gỡ. 7 ảnh Insights **không** lên landing. Hero không còn dòng badge dưới CTA; hệ sinh thái không còn dòng mô tả dưới tiêu đề.
...
- `apps/web/puck.config.tsx` đăng ký section + block theo categories cho editor kéo-thả Puck (`/admin/editor`). Editor full-height (không bọc `100dvh` trong khung cắt); font/màu ở **cột phải**. **Sửa/thêm section hoặc block phải cập nhật cả puck.config.tsx** (fields + defaultProps).
...
- Landing (`apps/web/app/page.tsx`) fetch `GET {API}/api/content/home`: có dữ liệu → render bằng Puck `<Render>`; không có dữ liệu → composition mặc định.
- Nội dung có lịch sử phiên bản (20 bản, khôi phục/reset trong `/admin/editor`).
- **Trợ lý chat**: `POST /api/chat/answer` ưu tiên AI thật (cấu hình tại `/admin/settings`) với ngữ cảnh kho kiến thức; AI tắt/lỗi thì khớp từ khóa (`/admin/knowledge`). Câu hỏi bảng giá được widget chặn: menu nhóm → `PackageDetailTable` thu gọn từ `lib/package-tables.ts` (`tablesForGroup`). Tin nhắn render markdown nhẹ (`ChatMarkdown`: **in đậm**, danh sách). Khung chat `data-lenis-prevent` + `min-h-0` để lăn chuột trong hội thoại, không bị Lenis nuốt. Mọi lượt ghi log; bảng giá lưu bản text. Admin `/admin/chat` xem đủ thread. Tag bấm nhanh vẫn hiện sau mỗi lượt.
- **Bảng giá**: nguồn duy nhất `apps/web/lib/pricing-data.ts`. UI chính là `pricing-board.tsx`: tab **Facebook / Bài đăng / TikTok / Khác**; mỗi gói một slide vuốt ngang. Nút ← → + bộ đếm overlay cố định trên thẻ tóm tắt (`package-carousel`). Không nút Hỏi Riviu trên thẻ gói. Gói nổi bật: badge **Phổ biến** góc phải, xoay chéo. `PackageDetailTable` luôn hiện dưới thẻ: cột **Dịch vụ / Thông tin (gạch đầu dòng) / Thành tiền** — không ĐVT, không STT. Hash `#goi-facebook` `#bai-dang` `#goi-tiktok` `#dich-vu-khac`. **Không đăng file PDF**.
- **Bằng chứng năng lực**: `apps/web/lib/network-data.ts` — 6 kênh + bài Insights (fallback `public/network/insight-post-*.webp`). Mỗi kênh có `url` Facebook; ảnh/thẻ bọc `<a target="_blank">`, field Puck “Link Facebook”. `top-posts.tsx` **chỉ** trên `/bang-gia`, hiện hết bài đã lưu. Ảnh/số/thêm-xóa tại `/admin/insights` (upload, không hiện path file) → Page slug `bang-gia`; fallback `TOP_POSTS`. ChannelNetwork + Stats sửa được từ Puck. Ảnh CMS: `<img>` + `mediaUrl()`, không `next/image`. Dùng `formatCompact` (làm tròn xuống).
- **Form liên hệ + inbox admin**: `ContactLead` (name, email, phone, message). `POST /api/contact` public; `GET /api/contact/leads` + `GET /api/admin/inbox` JWT. Trang `/admin/leads`. Toast + badge sidebar (poll ~8s, `localStorage` mốc đã xem khi vào leads/chat). Không email/Zalo khi admin đóng tab.
